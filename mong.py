...
def nueva_resena():
    ...
    try:
        ...
        opcion_c = input("Número de cliente: ")
 
        if not opcion_c.strip():
            print("Error: No ingresaste ninguna opción.")
            return

        idx_c = int(opcion_c) - 1
        
        if idx_c < 0 or idx_c >= len(clientes):
            print("Error: Opción de cliente no válida.")
            return
        ...
        opcion_p = input("Número de producto: ")
        idx_p = int(opcion_p) - 1
        
        if idx_p < 0 or idx_p >= len(productos):
            print("Error: Opción de producto no válida.")
            return
